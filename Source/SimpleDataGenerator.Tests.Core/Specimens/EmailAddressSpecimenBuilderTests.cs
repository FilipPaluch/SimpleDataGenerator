...
{
    [TestFixture]
    public class EmailAddressSpecimenBuilderTests
    {
        [Test]
        public void Should_Return_NoSpecimen_For_Not_Chosen_Property()
        {
            ...
            var sut = new EmailAddressSpecimenBuilder(chosenProperty);
        ...
        [Test]
        public void Should_Resolve_EmailAddress_For_Chosen_Proprety()
        {
            //ARRANGE

            var specimenContextMock = new Mock<ISpecimenContext>();

            specimenContextMock.Setup(x => x.Resolve(typeof (MailAddress))).Returns(new MailAddress("Test@gmail.com"));
            ...
            //ASSERT
            specimenContextMock.Verify(x => x.Resolve(typeof(MailAddress)), Times.Once());
